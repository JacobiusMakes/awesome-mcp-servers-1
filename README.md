...
- <img src="https://parlay-api.com/favicon.ico" height="14"/> [ParlayAPI](https://pypi.org/project/parlayapi-mcp/) - Real-time sports betting odds, player props, and prediction-market data (Kalshi, Polymarket) aggregated from 32+ sportsbooks, with a no-card free-tier signup so an assistant can provision an API key for its user in one call. `uvx parlayapi-mcp`
